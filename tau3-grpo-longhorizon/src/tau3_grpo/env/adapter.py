...
from tau3_grpo.tau2_bridge import airline_domain, flight_db_model, task_model
...
def adapt_record(
    ...
    *,
    ...
    if isinstance(instructions, dict):
        text = str(instructions.get("task_instructions", ""))
...
def load_default_flight_db() -> Any:
    ...
    from tau3_grpo.tau2_bridge import import_tau2
...
def environment_tool_names(environment: Any) -> list[str]:
    ...
    Used to check `configs/verl/tool_config.yaml` against the live tau2
    signatures. The OpenAI-shaped schemas come from
    `tau3_grpo.integration.tools.airline_tool_schemas`.
    """
