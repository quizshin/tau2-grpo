...
#
# Milestone D8–D11. All paths resolve against the four-directory `code/` root, so
# the script works from anywhere. GPU only: do not run this on a laptop.
#
# Usage:
#   scripts/run_train.sh e3 42
#   ARM=e3 SEED=42 TOTAL_UPDATES=60 scripts/run_train.sh
...
VAL_PARQUET="${VAL_PARQUET:-${PROJECT_ROOT}/data/parquet/airline_selection_seed${DATA_SPLIT_SEED}.parquet}"
TOOL_CONFIG="${PROJECT_ROOT}/configs/verl/tool_config.yaml"
INTERACTION_CONFIG="${PROJECT_ROOT}/configs/verl/interaction_config.yaml"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/${ARM}_seed${SEED}}"
...
MODEL_PATH="${MODEL_PATH:-${PROJECT_ROOT}/results/sft_airline_merged_seed42}"
...
if [[ "${MODEL_PATH}" == "${PROJECT_ROOT}"/* && ! -d "${MODEL_PATH}" ]]; then
  echo "error: merged SFT checkpoint missing: ${MODEL_PATH}" >&2
  echo "run: scripts/run_sft.sh && scripts/merge_sft.sh" >&2
...
if [[ -z "${CUSTOM_TRAIN_PARQUET}" ]]; then
  python -m tau3_grpo.cli.prepare_experiment \
    --arm "${ARM}" \
    --seed "${SEED}" \
...
python -m tau3_grpo.cli.generate_tool_config --output "${TOOL_CONFIG}"
...
export TAU3_GRPO_ANCHOR_HOOK="tau3_grpo.integration.anchor_hook:current_anchor"
...
exec python -m tau3_grpo.cli.train \
  ...
  trainer.project_name=tau3-grpo \
  trainer.experiment_name="${ARM}_seed${SEED}" \
