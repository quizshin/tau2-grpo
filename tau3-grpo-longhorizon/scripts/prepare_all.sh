...
python -m tau3_grpo.cli.prepare_data --seed "${DATA_SPLIT_SEED}" "${PREPARE_FLAGS[@]}"

echo "=== data split seed ${DATA_SPLIT_SEED}: parquet ==="
for SPLIT in train selection; do
  python -m tau3_grpo.cli.build_parquet --seed "${DATA_SPLIT_SEED}" --split "${SPLIT}"
done

echo "=== data split seed ${DATA_SPLIT_SEED}: 45+5 complete-dialogue SFT ==="
python -m tau3_grpo.cli.prepare_sft \
