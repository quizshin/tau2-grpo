...
export MODEL_PATH="${MODEL_PATH:-${PROJECT_ROOT}/results/sft_airline_merged_seed42}"
export RESULTS_DIR="${RESULTS_DIR:-${PROJECT_ROOT}/results/smoke/e0_seed42}"
export GROUP_SIZE=1
...
if [[ "${SMOKE_SKIP_USER_SIMULATOR_CHECK:-false}" != "true" ]]; then
  if ! curl --fail --silent --show-error --max-time 5 \
    ...
    echo "start scripts/serve_user_simulator.sh before this smoke" >&2
    exit 2
  fi
fi

exec "${PROJECT_ROOT}/scripts/run_train.sh" e0 42 \
