"""Config files must carry the frozen v2-2 values and the four-root paths."""
...
from tau3_grpo.algo.tau_gigpo import (  # noqa: E402
    DEFAULT_OMEGA,
    FNORM,
    GAMMA,
...
def test_base_config_gigpo_matches_frozen_constants():
    gigpo = _load("base_v2_2.yaml")["gigpo"]
...
def test_vllm_is_the_main_backend():
    assert _load("base_v2_2.yaml")["rollout"]["backend"] == "vllm"


def test_dynamic_filter_is_off_in_the_base_config():
    assert _load("base_v2_2.yaml")["dynamic_filter"]["enable"] is False


def test_four_arms_are_defined():
    arms = _load("arms.yaml")["arms"]
...
def test_anchor_ablation_covers_structured_and_db_hash():
    ablations = _load("arms.yaml")["ablations"]
...
def test_interaction_config_names_match_parquet_literal():
    config = _load("verl/interaction_config.yaml")
...
def test_tool_config_covers_read_and_write_tools():
    names = {
        ...
        for tool in _load("verl/tool_config.yaml")["tools"]
    }
    for required in (
        "get_user_details",
...
def test_tool_names_match_anchor_feature_lists():
    from tau3_grpo.anchors.features import CONFIRMATION_TOOLS, KNOWN_INFO_TOOLS
...
def test_train_script_calls_same_process_trainer_wrapper():
    text = _script("run_train.sh")
    assert "python -m tau3_grpo.cli.train" in text


def test_train_script_verifies_patches_first():
    assert "tau3_grpo.cli.verify_patches" in _script("run_train.sh")


def test_train_script_registers_the_estimator():
    wrapper = (PROJECT_ROOT / "src/tau3_grpo/cli/train.py").read_text(encoding="utf-8")
    assert "tau3_grpo.algo.verl_estimator" in wrapper
    assert 'runpy.run_module("verl.trainer.main_ppo"' in wrapper


def test_train_script_generates_live_tool_config():
    text = _script("run_train.sh")
    assert "tau3_grpo.cli.generate_tool_config" in text
...
def test_train_script_uses_four_root_relative_paths():
    text = _script("run_train.sh")
    assert 'CODE_ROOT="$(cd "${PROJECT_ROOT}/.." && pwd)"' in text
...
def test_training_seed_does_not_change_the_frozen_data_split():
    ...
    prepare = _script("prepare_all.sh")
...
def test_train_script_does_not_use_unsupported_rollout_seed():
    assert "actor_rollout_ref.rollout.seed" not in _script("run_train.sh")
...
def test_train_defaults_to_merged_sft_checkpoint_and_disables_shuffle():
    text = _script("run_train.sh")
    assert "results/sft_airline_merged_seed42" in text
    assert "data.shuffle=false" in text


def test_train_materialises_the_experiment_schedule():
    text = _script("run_train.sh")
    assert "tau3_grpo.cli.prepare_experiment" in text
...
def test_train_consumes_wrapper_positionals_before_hydra_overrides():
    text = _script("run_train.sh")
    assert text.count("  shift\n") == 2
    assert 'exec python -m tau3_grpo.cli.train' in text
...
def test_eval_script_uses_eval_temperature():
    assert "EVAL_TEMP=0.4" in _script("run_eval.sh")


def test_eval_script_goes_through_the_guard():
    assert "tau3_grpo.cli.evaluate" in _script("run_eval.sh")


def test_user_simulator_service_enables_prefix_caching():
    assert "--enable-prefix-caching" in _script("serve_user_simulator.sh")


def test_policy_serve_script_attests_checkpoint_content_and_pid():
    text = _script("serve_policy_eval.sh")
    assert "tau3_grpo.cli.attest_service" in text
    assert '--pid "$$"' in text
...
def test_a800_preflight_script_exists_and_checks_cuda_contract():
    path = PROJECT_ROOT / "scripts/smoke/check_installation.sh"
    ...
    assert "tau3_grpo.cli.verify_patches" in text


def test_one_trajectory_smoke_is_isolated_from_formal_defaults():
    path = PROJECT_ROOT / "scripts/smoke/run_one_trajectory.sh"
    text = path.read_text(encoding="utf-8")
    ...
    assert "results/smoke/e0_seed42" in text
...
def test_setup_requires_python_312_for_both_environments():
    text = (PROJECT_ROOT.parent / "setup.sh").read_text(encoding="utf-8")
...
def test_sft_config_matches_frozen_45_plus_5_budget():
    config = _load("sft_airline_lora.yaml")
...
def test_sft_scripts_require_python312_torch28_cuda128():
    ...
    assert 'torch.__version__.startswith("2.8")' in text
    assert 'torch.version.cuda == "12.8"' in text
    assert "tau3_grpo.cli.sft_train" in text
