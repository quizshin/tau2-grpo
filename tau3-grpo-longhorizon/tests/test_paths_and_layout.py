"""Four-root layout and version pins."""
...
def test_code_root_resolves_to_four_root_parent():
    assert CODE_ROOT.is_dir()
    assert PROJECT_ROOT.name == "tau3-grpo-longhorizon"
    assert PROJECT_ROOT.parent == CODE_ROOT


def test_all_four_roots_exist():
    verify_four_root_layout()
    for name in EXPECTED_ROOTS:
        assert (CODE_ROOT / name).is_dir(), f"missing root: {name}"


def test_expected_roots_are_exactly_four():
    assert len(EXPECTED_ROOTS) == 4
    assert set(EXPECTED_ROOTS) == {
        "tau3-grpo-longhorizon",
        "env_info",
        "tau2-bench",
        "verl",
    }
...
def test_verify_layout_reports_missing(tmp_path):
    with pytest.raises(FileNotFoundError, match="missing required directories"):
        verify_four_root_layout(tmp_path)
