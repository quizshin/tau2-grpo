...
from tau3_grpo.env.adapter import AdaptedTask  # noqa: E402
from tau3_grpo.env.session import SessionFactory, UserSimulatorConfig  # noqa: E402
...
def factory(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("tau3_grpo.env.session.build_environment", _StubEnvironment)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("tau3_grpo.env.session.user_simulator_cls", lambda: _StubUser)
...
def interaction(factory):
    from tau3_grpo.integration.interaction import Tau3AirlineInteraction
    from tau3_grpo.integration.registry import SESSIONS
...
def test_turn_limit_terminates(interaction, factory):
    from tau3_grpo.env.session import SessionLimits
    from tau3_grpo.integration.registry import SESSIONS, SessionEntry
...
def _tool(name: str):
    ...
    from tau3_grpo.integration.tools import Tau3AirlineTool
