...
class MatchedBudget:
    ...
    def __post_init__(self):
        if not math.isfinite(self.started_at) or (self.budget_seconds is not None and not math.isfinite(self.budget_seconds)):
            raise ValueError('Budget times must be finite')
        if (self.budget_seconds is not None and self.budget_seconds <= 0) or self.interval <= 0:
    ...
    def from_environment(cls, *, save_freq, test_freq):
        ...
        stop_path = os.environ.get('TAU3_STOP_REQUEST_PATH')
        if not seconds and not stop_path:
            return None
        if seconds and os.environ.get('TAU3_GRPO_ARM') != 'e0':
        ...
        return cls(started_at=float(os.environ.get('TAU3_BUDGET_STARTED_AT', time.time())),
                   budget_seconds=float(seconds) if seconds else None, interval=interval,
                   state_path=Path(os.environ['TAU3_BUDGET_STATE_PATH']),
                   stop_request_path=Path(stop_path) if stop_path else None)
    ...
    def observe(self, step, *, now=None, ceiling=None):
        ...
        if self.stop_request_path and self.stop_request_path.exists() and not self.stop_requested:
            self.stop_requested = True
            boundary = ((step + self.interval - 1) // self.interval) * self.interval
            self.target_step = min(self.target_step or boundary, boundary)
            self.observed_step = step
        if self.target_step is None and ((self.budget_seconds is not None and elapsed >= self.budget_seconds) or step == ceiling):
        ...
        state = {'mode': 'manual_stop_at_boundary' if self.stop_requested else 'e0_time_then_round_up_shared_steps' if self.budget_seconds else 'fixed_steps',
                 'stop_requested': self.stop_requested, 'completed_step': step,
