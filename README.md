...
2026-09-14：`code` 已加入 [v1/v2/v3 锚点配置开关与真实 DB 无卡重放](docs/anchor_switch_and_replay_20260914.md)。默认仍为 v1，候选配置未启动训练；5090 配置基线另存于 `config/5090-20260914` 分支。

后续新增 [v4 决策状态与弃权实验](docs/decision_state_v4_20260914.md)：机制测试通过，但真实对话全部弃权，暂不建议正式训练。

现已加入 [真实对话字段抽取与证据校验](docs/grounded_evidence_20260914.md)：无卡审计原文出处、工具事实及支付对应；字段证据不等于完整语义状态，尚未接入训练归并。

按“假设已有语义模型”的要求，新增 [目标、条件与确认范围的模拟归并链路](docs/semantic_model_mock_20260914.md)：验证语义事件到 GiGPO 步级优势的后端逻辑，未调用真实模型或更改训练默认值。
