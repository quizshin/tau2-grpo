...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        dist.init_process_group("nccl", init_method="file://" + str(Path(tmp) / "rendezvous"),
                                rank=0, world_size=1, timeout=datetime.timedelta(seconds=90))
        try:
            wrapped = FSDP(model, device_id=torch.device("cuda", 0),
            ...
            optimizer.step()
            assert not torch.equal(before, tracked.detach().cpu()), "adapter did not update"
            torch.cuda.synchronize()
            report.update(loss=loss.item(), grad_norm=norm.item(), optimizer_change=True,
            ...
            (output / "rl-memory.json").write_text(json.dumps(report, indent=2) + "\n")
            print(json.dumps(report), flush=True)
        finally:
