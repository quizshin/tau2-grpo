...
def run_case(length, seed):
    ...
    for mode in modes:
        ...
        try:
            ...
            print(json.dumps({'length':length,'mode':mode,'stage':'forward_start'}),flush=True)
            with (tracker if mode.startswith('native') else nullcontext()), torch.autocast('cuda', dtype=torch.bfloat16, enabled=mode == 'native_autocast'):
                if mode == 'oracle_fp64':
            ...
            print(json.dumps({'length':length,'mode':mode,'stage':'backward_start'}),flush=True)
            loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--triton-fp32',choices=['default','ieee','tf32x3'],default='default')
    ...
    result={'kind':'synthetic_kernel_diagnosis_not_RL','torch':torch.__version__,
        'gpu':torch.cuda.get_device_name(),'triton_fp32':args.triton_fp32,'cases':[]}
