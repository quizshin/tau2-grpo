...
def run(args):
    ...
    report={'kind':'fixed_64_RL_trajectories_real_PPO_actor_replay_not_online_training',
        ...
        'study':args.study,'timing_only':args.timing_only,'fa2_deterministic_backward':args.fa2_deterministic,
        'scoring':{},'updates':{},'failures':[]}
    if args.study=='fa2':
        report['precision']='FP32 parameters/projections/GDN IEEE; BF16 standard-attention inputs and outputs cast back to FP32'
        report['mode_definitions']={'native_padded':'cached native FP32 padded numerical anchor',
            'native_trim':'FLA IEEE plus BF16-input PyTorch SDPA standard attention',
            'fla_trim':'FLA IEEE plus external FA2 with identical BF16 standard-attention inputs'}
    ...
    def configure_mode(mode):
        ...
        if args.study=='fa2':
            selected='fla_fp32'
            install_standard_attention_backend('fa2_bf16' if mode=='fla_trim' else 'sdpa_bf16',fa2_deterministic=args.fa2_deterministic)
