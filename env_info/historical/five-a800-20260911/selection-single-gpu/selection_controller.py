...
R=Path('/root/autodl-tmp/tau3-5xa800-20260911');W=R/'selection-single-gpu'
...
def launch(cmd,name):
 ...
 p=subprocess.Popen(cmd,env=os.environ.copy(),stdout=log,stderr=subprocess.STDOUT,start_new_session=True,cwd=R/'code');log.close();processes.append(p);return p
...
plan={'gpu':0,'methods':['off','on'],'tasks':60,'trials':4,'concurrency':2,'policy_temperature':.4,'user_temperature':0,'max_new_tokens_per_turn':2048,'policy_context':24576,'user_context':16384,'max_steps':30,'thinking_on_changes_inference_mode':True}
(W/'plan.json').write_text(json.dumps(plan,indent=2))
try:
 sim=launch(serve(SIM,R/'model_store/Qwen3.8-27B-AWQ-INT4','tau3-user',8100,.42,16384,False,True),'simulator');ready(sim,8100)
 for method in ['off','on']:
  checkpoint=W/('merged-'+method)
  p=launch(serve(PY,checkpoint,'tau3-policy',8000,.38,24576,method=='on'),'policy-'+method);ready(p,8000)
 ...
 (W/'controller.exit').write_text('0');print('Both selection runs completed',flush=True)
