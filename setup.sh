...
install_source_path_file() {
  ...
  printf '%s\n%s\n%s\n' \
    "${ROOT_DIR}/tau3-grpo-longhorizon/src" \
...
case "${MODE}" in
  cpu-test)
    ...
    "${VENV_DIR}/bin/pip" install -e "${ROOT_DIR}/tau3-grpo-longhorizon[dev,data]"
  ...
  a800)
    ...
    "${VENV_DIR}/bin/pip" install -e "${ROOT_DIR}/tau3-grpo-longhorizon[dev,data,sft]"
    install_source_path_file "${VENV_DIR}"
    ;;
  *)
