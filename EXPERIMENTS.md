...
## 2026-09-22 MT-GTPO 调用局部信用设计

状态 `planned`。固定 `call_local_v1` 候选：保留原长程项和非调用文本优势，仅在归属可认证的工具调用区间替换即时信用，使用按轨迹等权的调用奖励基线。已对照现有数组核心、奖励和 veRL 接口，并核对构造数值例子；尚未实现 estimator、完成全量离线比较或启动 GPU。公式、数据契约、回退、验收与局限见 [候选设计](docs/mt_gtpo_call_local_credit_design_20260922.md)。既有 v4 5090 配置不包含此候选。

## 2026-09-22 call_local_v1 两步GPU smoke完成

用户指定test01在8×5090完成2步128条，A45起点、split-v4未校准默认初始权重、DF off、token budget v1。约29分钟；梯度1.846/2.376，累计改变38,311个token，两步CPU重算误差0，24份卷积权重同步审计全匹配；完整step2检查点及SwanLab1/2回读通过。相同batch旧turn与call-local对比：错误正优势17→15，正奖励调用正优势114→81。无独立评测，不能判断模型收益；没有追加训练。训练服务已退出。[结果与保存位置](results/runs/mt_call_local_smoke/test01/report.md)。

## 2026-09-22 MT-GTPO 5090 最小配置候选

新增 `formal50_5090_a45_mt_gtpo_v4.yaml`，复用已完成 GRPO 的八卡 FSDP2/A45 路径，组合 MT-GTPO 与 split-v4 过程奖励。本地 CPU 配置加载检查通过；未部署、未启动 GPU，当前没有通过校准的冻结配方证据。入口和限制见 [最小接入说明](docs/mt_gtpo_5090_minimal_20260922.md)。

## 2026-09-22 8×5090 A45 GRPO 30-step 完成

v4 已完成 30 步；第 10/20/30 步各 240 条评测分别成功 110/108/130（45.83%/45.00%/54.17%）。step30 比 A45 SFT 高 8.33 个百分点，经验 pass@4 同为 46/60；单次实验，不宣称稳定提升。完整最新 step30 检查点与恢复元数据验收通过，SwanLab 云端独立回读确认 1–30 步。重要产物核实位于 NFS 共享盘，小型结果已本地保存并逐文件校验。09:01 Chrome 已观察到目标实例“已关机”，自动检查已停止；不推断关机执行者。[最终验收](docs/grpo_5090_a45_u30_20260921.md)。以下保留启动与故障历史。
...
## 2026-09-22：5090 call-local 两步原始轨迹 CPU 回放

完成128轨迹/1246轮/864调用固定组回放；两份原始输入hash匹配GPU审计，奖励与优势重算误差0，独立公式分解误差不超过3.25e-15，非调用token及returns不变。正奖励调用正优势114→81，35次丢失中32次来自成功轨迹，且35次均为轮内同奖励；错误调用正优势17→15（3次纠正、1次新增）。局部基线替换破坏部分即时进展与未来剩余项的抵消，不建议扩训当前call_local_v1。保留25条未评分状态，不作为官方失败。原turn_v1+split-v4可考虑另做30步未校准探索，但当前runner尚无这种执行入口，未启动新GPU训练或修改算法。详见[完整回放](docs/mt_gtpo_call_local_replay_20260922.md)。


## 2026-09-22：轮内调用残差候选 CPU 诊断

固定原5090两步864调用，计算前登记beta=1的 A_call=A_turn+(q-同轮调用均值)/(原return标准差+eps)，保留原guard；未修改训练实现。此前35次正奖励信号损失全部恢复；正奖励正优势114→117，新增3次来自失败2次/未评分1次，不能作收益结论；错误正优势仍17次。仅6个轮/17次调用调整，单调用与全同奖励轮不变，调用等权均值保留但token加权/PPO梯度不守恒。时间信用风险尚未解决，无GPU。详见[候选诊断](docs/mt_gtpo_call_residual_probe_20260922.md)。


## 2026-09-22：split-v4 未校准探索训练入口

后续执行更新（11:09，北京时间）：用户授权后已部署并启动 `mt_gtpo_5090_a45/v4-explore-u30-01`，控制器 PID 18853。远端 CPU 45 passed、真实 dry-run 通过，正在初始化 GPU 服务；尚无完成更新或评测结论。只同步 runner/profile，原文件已备份；详细证据见下方探索文档。以下未部署状态为初版交付记录。

按用户要求新增显式 --uncalibrated-exploration，只开放 MT-GTPO turn_v1+paper_env_split_v4 的十步倍数实验，保留正式 IRC 门禁。30步配置从A45新起、8×8候选/步，每10步保存与selection60×4评测；日志与Hydra/SwanLab明确标记未校准探索。续训要求同run同模式、实际奖励及优势设置一致；不允许从call-local smoke接续。本地相关64项CPU通过、lint零新增、diff检查通过；首轮PATH环境问题和测试名称插值修正详见记录。未部署5090、未启动GPU；运行所需两文件部署包已生成。详见[探索入口与命令](docs/mt_gtpo_v4_exploration_20260922.md)。
