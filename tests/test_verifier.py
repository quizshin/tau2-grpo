...
@pytest.mark.tau3
@pytest.mark.parametrize("perform_action", [False, True])
def test_injected_db_replay_scores_missing_and_correct_write(requires_tau2, perform_action):
    ...
    initial_hash = db.get_hash()
    reservation_id = next(iter(db.reservations))
    arguments = {"reservation_id": reservation_id}
    task = SimpleNamespace(initial_state=None, evaluation_criteria=EvaluationCriteria(
        actions=[Action(action_id="cancel", name="cancel_reservation", arguments=arguments)],
        reward_basis=[RewardType.DB],
    ))
    messages = []
    if perform_action:
        call = ToolCall(id="call_cancel", name="cancel_reservation", arguments=arguments)
        env = get_environment(db=db)
        messages = [AssistantMessage(role="assistant", tool_calls=[call]), env.get_response(call)]
    for _ in range(2):
        result = EnvironmentEvaluator.calculate_reward(
            environment_constructor=get_environment, task=task, full_trajectory=messages,
            env_kwargs={"db": db},
        )
        assert result.reward == float(perform_action)
        assert result.db_check.db_match is perform_action
        assert db.get_hash() == initial_hash
...
def test_build_simulation_run_matches_tau2_model(requires_tau2):
    from tau3_grpo.evaluation.verifier import build_simulation_run
...
def test_evaluation_type_all_exists(requires_tau2):
    from tau3_grpo.envs.tau2_bridge import evaluator
...
def test_termination_reason_enum_covers_our_categories(requires_tau2):
    from tau3_grpo.envs.tau2_bridge import simulation_models
...
def test_verify_trajectory_replays_record_specific_db(monkeypatch, tmp_path):
    ...
    from tau3_grpo.evaluation import verifier as module
