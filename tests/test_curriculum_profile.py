...
@pytest.mark.parametrize("phase,updates", [("pilot", 10), ("extend", 50)])
def test_curriculum_budget_and_resume_profile(phase, updates, tmp_path):
    config = CODE_ROOT / f"configs/train/rl/qwen35_4b_full_a800_curriculum40_{phase}_20260912.yaml"
    ...
    assert env["TRAIN_MANIFEST_DIR"].endswith("rl_curriculum_20260912/manifests")
    assert (
