...
from tau3_grpo.envs.adapter import AdaptedTask
from tau3_grpo.envs.registry import SessionEntry, SessionRegistry
...
def stub_session(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tau3_grpo.envs.session.build_environment", lambda db: _StubEnvironment(db)
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "tau3_grpo.envs.session.user_simulator_cls", lambda: _StubUserSimulator
    )
