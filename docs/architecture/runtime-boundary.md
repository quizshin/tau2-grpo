...
| loader | `data.dataset` → `envs.adapter.adapt_record` | `data.official.load_official_airline_tasks` |
...
- `assert_trainable_entries` additionally rejects any entry whose split is
  `tau3_final`.
- `experiments.winner_lock.assert_final_run_allowed` refuses the final run unless a
...
- `evaluation.service_attestation` binds the vLLM exec PID, served model name,
...
`evaluation.verifier` builds an official `SimulationRun` from the session's recorded
