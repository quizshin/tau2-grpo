...
Supports **Qwen2.5 and Qwen3.5 series models** as text-only tool agents and
user simulators. Trains on AReaL's 1,148 synthetic τ²-style Airline tasks with
veRL, picks a winner on a frozen 60-task internal split, then evaluates that
single frozen winner on the 50 official Airline tasks from τ³-bench v1.0.1.

See the [model-family guide](qwen35.md) for compatible environments and
launchers. Qwen3.5 support covers dense models; the current two-A800 example
uses a 0.8B policy and a 4B simulator. The commands and GPU results below
document the existing Qwen2.5 experiment profile.

The [Qwen3.8 quantized simulator guide](qwen38_simulator.md) covers a separate
27B INT4 inference environment and pinned download; this does not change policy
model support. See the [two-A800 RL budget](rl_model_budget_2xa800.md) before
choosing full-parameter or LoRA training for larger policies.
...
τ³ source on every training path, and `experiments.winner_lock` refuses to run the
...
`code` is the project root: `tau3_grpo/` contains business code, while
`configs/`, `scripts/`, `tests/` and `docs/` are directly accessible.
The sibling `verl/` and `tau2-bench/` directories contain vendored dependencies.
See [layout and entrypoints](architecture/layout.md) for the complete directory map.
...
python -m tau3_grpo.data.prepare_sft --seed 42 \
...
scripts/train/sft/run_base.sh
scripts/train/sft/merge.sh
...
scripts/serve/simulator_base.sh
...
`python -m tau3_grpo.training.rl.train`; that wrapper registers the
...
```bash
scripts/train/rl/merge_checkpoint.sh \
...
python -m tau3_grpo.experiments.freeze_winner --experiment e3 \
  --checkpoint results/e3_seed42/global_step_40/merged_hf \
  --metric selection_solve_rate --score 0.42 --task-count 60 --seeds 42 43

scripts/serve/policy.sh results/e3_seed42/global_step_40/merged_hf
...
```bash
python -m tau3_grpo.evaluation.report \
